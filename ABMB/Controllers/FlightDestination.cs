...
public class FlightDestination : ControllerBase
{
    ...
    {
        if (
            string.IsNullOrWhiteSpace(departure_id)
            || string.IsNullOrWhiteSpace(arrival_id)
            || string.IsNullOrWhiteSpace(departure_date)
            || string.IsNullOrWhiteSpace(return_date)
            || adults < 1
        )
        {
