...
public class CsvService
{
    ...
    public CsvService(IDbContextFactory<AppDbContext> contextFactory, ILogger<CsvService> logger)
    {
        ...
        _logger.LogInformation(
            $"CSV Service initialized with parallelism degree: {_maxDegreeOfParallelism}"
        );
    }
    ...
    public async Task<IEnumerable<OldFlight>> ReadCsvFile(Stream fileStream)
    {
        try
        {
            ...
            using (var csv = new CsvReader(reader, config))
            {
                ...
                _logger.LogInformation(
                    $"Created {batches.Count} batches for processing with {_maxDegreeOfParallelism} threads"
                );
                ...
                for (int i = 0; i < _maxDegreeOfParallelism; i++)
                {
                    var thread = new Thread(() =>
                    {
                        ...
                        {
                            _logger.LogInformation(
                                $"Thread {Thread.CurrentThread.ManagedThreadId} started"
                            );
                            ...
                            while (remainingBatches.TryDequeue(out var batch))
                            {
                                ...
                                {
                                    _logger.LogDebug(
                                        $"Thread {Thread.CurrentThread.ManagedThreadId} processing batch of {batch.Count} records"
                                    );
                                    ...
                                    _logger.LogDebug(
                                        $"Thread {Thread.CurrentThread.ManagedThreadId} completed batch"
                                    );
                                }
                                catch (Exception ex)
                                {
                                    _logger.LogError(
                                        ex,
                                        $"Error in thread {Thread.CurrentThread.ManagedThreadId} while processing batch"
                                    );
                                }
                            }

                            _logger.LogInformation(
                                $"Thread {Thread.CurrentThread.ManagedThreadId} finished - no more batches"
                            );
                        ...
                        catch (Exception ex)
                        {
                            _logger.LogError(
                                ex,
                                $"Unhandled exception in thread {Thread.CurrentThread.ManagedThreadId}"
                            );
                            lock (lockObj)
                            {
                                completedThreads++;
                ...
                _logger.LogInformation(
                    $"All {_maxDegreeOfParallelism} threads completed, saved {results.Count} records"
                );
                return results;
            }
        }
        ...
        using (var context = _contextFactory.CreateDbContext())
        {
            using (var transaction = await context.Database.BeginTransactionAsync())
            {
                try
                {
                    ...
                    var existingFlights = await context
                        .OldFlights.Where(f => batchIds.Contains(f.Id))
                        .ToListAsync();
                    ...
                    when (ex.InnerException is PostgresException pgEx && pgEx.SqlState == "23505")
                {
                    _logger.LogError(
                        ex,
                        "Duplicate key value violates unique constraint 'PK_OldFlights'"
                    );
